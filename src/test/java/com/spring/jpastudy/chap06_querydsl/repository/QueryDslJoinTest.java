...
class QueryDslJoinTest {
    ...
    void innerJoinTest() {
        ...
        List<Tuple> idolList = factory
                .select(idol, group)
                .from(idol)
                //첫 번째 파라미터는 from 절에 있는 entity에 있는 연관 객체
        ...
        for (Tuple tuple : idolList) {
            ...
            Group foundGroup = tuple.get(idol.group);
    ...
    @Test
    @DisplayName("22세 이상인 그룹 그룹명과 평균 나이 조회하기")
    void quiz2() {
        //given
    ...
    void practice3quiz() {
        ...
        result.forEach(tuple -> {
            ...
            System.out.printf("\n# 아이돌명: %s, 그룹명: %s, 앨범명 : %s, 발매연도 : %d년 \n\n"
                    , foundidol.getIdolName(), foundidol.getGroup().getGroupName(), foundAlbum.getAlbumName(), foundAlbum.getReleaseYear());
        });
    }
